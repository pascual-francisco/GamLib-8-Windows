...
namespace render
{
	TileMapManager::TileMapManager(GLuint tilesPerMap, GLuint tilesPerPage, GLuint* mapTilesData, GLuint tileWidth, GLuint tileHeight, GLuint tileSeparationX, GLuint tileSeparationY, GLuint viewPortWidth, GLuint viewPortHeight, GLuint program, GLenum mode)
	{
		...
		ptrvShaLayout = nullptr;
		ptrvCol1Layout = nullptr;
		ptrvCol2Layout = nullptr;
		ptrvAlpLayout = nullptr;
	...
	TileMapManager::~TileMapManager()
	{
		...
		delete ptrvShaLayout;
		delete ptrvCol1Layout;
		delete ptrvCol2Layout;
		delete ptrvAlpLayout;
	...
	void TileMapManager::initLayouts()
	{
		...
		Entity 0
			Sprite 0
			[XYZSTPQSSSSRGBARGBARGBANNNTTTSSSRRR]
			[XYZSTRQSSSSRGBARGBARGBANNNTTTSSSRRR]
			[XYZSTRQSSSSRGBARGBARGBANNNTTTSSSRRR]
			[XYZSTRQSSSSRGBARGBARGBANNNTTTSSSRRR]
			= 35 Floats
			35 Float * 4 Vertices = 140 Floats
			1 Quad = 140 Floats
		*/
		ptrvPosLayout = new VertexBufferLayout(programID, "vs_vertex_position", 3, GL_FALSE, 35, 0 * 4);
		ptrvTexLayout = new VertexBufferLayout(programID, "vertex_texture_coords", 4, GL_FALSE, 35, 3 * 4);
		ptrvShaLayout = new VertexBufferLayout(programID, "vertex_shader", 4, GL_FALSE, 35, 7 * 4);
		ptrvCol1Layout = new VertexBufferLayout(programID, "vertex_color1", 4, GL_FALSE, 35, 11 * 4);
		ptrvCol2Layout = new VertexBufferLayout(programID, "vertex_color2", 4, GL_FALSE, 35, 15 * 4);
		ptrvAlpLayout = new VertexBufferLayout(programID, "vertex_alpha", 4, GL_FALSE, 35, 19 * 4);
		ptrvNorLayout = new VertexBufferLayout(programID, "vertex_normal", 3, GL_FALSE, 35, 23 * 4);
		ptrvTraLayout = new VertexBufferLayout(programID, "vertex_translate", 3, GL_FALSE, 35, 26 * 4);
		ptrvScaLayout = new VertexBufferLayout(programID, "vertex_scale", 3, GL_FALSE, 35, 29 * 4);
		ptrvRotLayout = new VertexBufferLayout(programID, "vertex_rotate", 3, GL_FALSE, 35, 32 * 4);
	...
	void TileMapManager::updateVertexArray(Sprite* sprite, GLuint offset)
	{
		...
		updateShader(sprite, offset + 7);
		updateColor1(sprite, offset + 11);
		updateColor2(sprite, offset + 15);
		updateAlpha(sprite, offset + 19);
		updateNormal(sprite, offset + 23);
		updateTranslate(sprite, offset + 26);
		updateScale(sprite, offset + 29);
		updateRotate(sprite, offset + 32);
	}
	...
	void render::TileMapManager::updateColor1(Sprite* sprite, GLuint offset)
	{
	...
	void render::TileMapManager::updateColor2(Sprite* sprite, GLuint offset)
	{
	}

	void render::TileMapManager::updateAlpha(Sprite* sprite, GLuint offset)
	{
	}


	void render::TileMapManager::updateShader(Sprite* sprite, GLuint offset)
	{
	}

	void render::TileMapManager::updateNormal(Sprite* sprite, GLuint offset)
	{
	}
